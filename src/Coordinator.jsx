...
export default function Coordinator(){
    ...
    const trainDetails = trains.map((train)=>{
        if (train.capacity > 0){
            return( <h4 className="train">{train.name} : Remaining Capacity {train.capacity}</h4>)
        }
    ...
    const orderDetails = orders.map((order) =>{
            return (
                <div key={order.order_id} className="order">
                    ...
                    <select value = {order.train} onChange={(event)=> handleChange(event  , order.order_id ,order.size )}>
                        <option value="-1">-Select Train-</option>
                        {trainOption}
                    </select>
                    <button className="train-submit" onClick={() => handleConfirm(order.order_id, order.train , order.size)}>Confirm</button>
                </div>
            )      
    })
